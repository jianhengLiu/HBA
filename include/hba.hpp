...
class LAYER {
public:
  int pose_size, layer_num, max_iter, part_length, left_size, left_h_size,
  ...
  void init_storage(int total_layer_num_) {
    ...
    if (layer_num < total_layer_num_) {
      int hessian_size =
          (thread_num - 1) * (WIN_SIZE - 1) * WIN_SIZE / 2 * part_length;
  ...
  void init_parameter(int pose_size_ = 0) {
    ...
    while (part_length == 0 || (gap_num - (thread_num - 1) * part_length + 1) /
                                       double(part_length) >
                                   2) {
      thread_num -= 1;
      part_length = ceil((gap_num + 1) / double(thread_num));
    ...
    if (tail == 0) {
      ...
      left_h_size =
          (gap_num - (thread_num - 1) * part_length) * GAP + WIN_SIZE - 1;
      j_upper = gap_num - (thread_num - 1) * part_length + 1;
    } else {
      left_size = (gap_num - (thread_num - 1) * part_length + 1) * WIN_SIZE +
                  GAP + tail;
      left_h_size = (gap_num - (thread_num - 1) * part_length + 1) * GAP +
                    last_win_size - 1;
      j_upper = gap_num - (thread_num - 1) * part_length + 2;
    }

    printf("init parameter:\n");
    printf("layer_num %d | thread_num %d | pose_size %d | max_iter %d | "
           "part_length %d | gap_num %d | last_win_size %d | "
           "left_gap_num %d | tail %d | left_size %d | left_h_size %d | "
           "j_upper %d | "
           "downsample_size %f | voxel_size %f | eigen_ratio %f | reject_ratio "
           "%f\n",
           layer_num, thread_num, pose_size, max_iter, part_length, gap_num,
           last_win_size, left_gap_num, tail, left_size, left_h_size, j_upper,
...
public:
  ...
  HBA(int total_layer_num_, std::string pcl_path_, std::string pose_path_,
    ...
    for (int i = 1; i < total_layer_num; i++) {
      int pose_size_ =
          (layers[i - 1].thread_num - 1) * layers[i - 1].part_length;
      pose_size_ += layers[i - 1].tail == 0 ? layers[i - 1].left_gap_num
    ...
    for (int i = 0; i < layers[cur_layer_num].thread_num; i++)
      if (i < layers[cur_layer_num].thread_num - 1)
        for (int j = 0; j < layers[cur_layer_num].part_length; j++) {
          ...
          layers[cur_layer_num + 1]
              .pose_vec[i * layers[cur_layer_num].part_length + j] =
  ...
  void pose_graph_optimization() {
    ...
    initial.insert(0,
                   gtsam::Pose3(gtsam::Rot3(init_pose[0].q.toRotationMatrix()),
    ...
    for (uint i = 0; i < init_pose.size(); i++) {
      if (i > 0)
        initial.insert(
            i, gtsam::Pose3(gtsam::Rot3(init_pose[i].q.toRotationMatrix()),
      ...
      if (i % GAP == 0 && cnt < init_cov.size())
        for (int j = 0; j < WIN_SIZE - 1; j++)
          for (int k = j + 1; k < WIN_SIZE; k++) {
            ...
            Vector6 << fabs(1.0 / init_cov[cnt - 1](0)),
                fabs(1.0 / init_cov[cnt - 1](1)),
                fabs(1.0 / init_cov[cnt - 1](2)),
                fabs(1.0 / init_cov[cnt - 1](3)),
                fabs(1.0 / init_cov[cnt - 1](4)),
                fabs(1.0 / init_cov[cnt - 1](5));
    ...
    for (int i = 0; i < pose_size - 1; i++)
      for (int j = i + 1; j < pose_size; j++) {
        ...
        Vector6 << fabs(1.0 / upper_cov[cnt - 1](0)),
            fabs(1.0 / upper_cov[cnt - 1](1)),
            fabs(1.0 / upper_cov[cnt - 1](2)),
            fabs(1.0 / upper_cov[cnt - 1](3)),
            fabs(1.0 / upper_cov[cnt - 1](4)),
            fabs(1.0 / upper_cov[cnt - 1](5));
        gtsam::noiseModel::Diagonal::shared_ptr odometryNoise =
            gtsam::noiseModel::Diagonal::Variances(Vector6);
        gtsam::NonlinearFactor::shared_ptr factor(
            new gtsam::BetweenFactor<gtsam::Pose3>(
                i * pow(GAP, total_layer_num - 1),
                j * pow(GAP, total_layer_num - 1), gtsam::Pose3(R_sam, t_sam),
                odometryNoise));
        graph.push_back(factor);
      }
