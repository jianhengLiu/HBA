...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT *> &feat_map,
               pcl::PointCloud<PointType> &feat_pt, Eigen::Quaterniond q,
               Eigen::Vector3d t, int fnum, double voxel_size, int window_size,
               float eigen_ratio) {
  float loc_xyz[3];
...
void parallel_comp(LAYER &layer, int thread_id, LAYER &next_layer) {
  ...
  for (int i = thread_id * part_length; i < (thread_id + 1) * part_length;
       i++) {
    ...
    for (int loop = 0; loop < layer.max_iter; loop++) {
      ...
      for (size_t j = 0; j < WIN_SIZE; j++) {
        ...
        cut_voxel(surf_map, *src_pc[j], Eigen::Quaterniond(x_buf[j].R),
                  x_buf[j].p, j, layer.voxel_size, WIN_SIZE, layer.eigen_ratio);
      }
      ...
      if (loop > 0 &&
              abs(residual_pre - residual_cur) / abs(residual_cur) < 0.05 ||
          loop == layer.max_iter - 1) {
    ...
    for (size_t j = 0; j < WIN_SIZE; j++) {
      ...
      pcl::PointCloud<PointType>::Ptr pc_oneframe(
...
void parallel_tail(LAYER &layer, int thread_id, LAYER &next_layer) {
  ...
  double load_t = 0, undis_t = 0, dsp_t = 0, cut_t = 0, recut_t = 0,
         total_t = 0, tran_t = 0, sol_t = 0, save_t = 0;

  if (layer.gap_num - (layer.thread_num - 1) * part_length + 1 != left_gap_num)
    printf("THIS IS WRONG!\n");

  for (uint i = thread_id * part_length;
       i < thread_id * part_length + left_gap_num; i++) {
    ...
    for (int loop = 0; loop < layer.max_iter; loop++) {
      ...
      for (size_t j = 0; j < WIN_SIZE; j++) {
        ...
        t0 = ros::Time::now().toSec();
        cut_voxel(surf_map, *src_pc[j], Quaterniond(x_buf[j].R), x_buf[j].p, j,
                  layer.voxel_size, WIN_SIZE, layer.eigen_ratio);
    ...
    for (int loop = 0; loop < layer.max_iter; loop++) {
      ...
      for (size_t j = 0; j < layer.last_win_size; j++) {
        if (layer.downsample_size > 0)
          downsample_voxel(*src_pc[j], layer.downsample_size);
        cut_voxel(surf_map, *src_pc[j], Quaterniond(x_buf[j].R), x_buf[j].p, j,
                  layer.voxel_size, layer.last_win_size, layer.eigen_ratio);
      }
      for (auto iter = surf_map.begin(); iter != surf_map.end(); ++iter)
        iter->second->recut();
      ...
      if (loop > 0 &&
        ...
        for (int j = 0; j < layer.last_win_size * (layer.last_win_size - 1) / 2;
             j++)
          layer.hessians[i * (WIN_SIZE - 1) * WIN_SIZE / 2 + j] = hess_vec[j];
  ...
  printf("load pcd %.2fs %.2f%% | undistort pcd %.2fs %.2f%% | "
         "downsample %.2fs %.2f%% | cut voxel %.2fs %.2f%% | recut %.2fs "
         "%.2f%% | trans %.2fs %.2f%% | solve %.2fs %.2f%% | "
         "save pcd %.2fs %.2f%%\n",
         load_t, load_t / total_t * 100, undis_t, undis_t / total_t * 100,
         dsp_t, dsp_t / total_t * 100, cut_t, cut_t / total_t * 100, recut_t,
         recut_t / total_t * 100, tran_t, tran_t / total_t * 100, sol_t,
...
void global_ba(LAYER &layer) {
  ...
  for (int loop = 0; loop < layer.max_iter; loop++) {
    ...
    cout << "Residual absolute: " << abs(residual_pre - residual_cur) << " | "
         << "percentage: "
         << abs(residual_pre - residual_cur) / abs(residual_cur) << endl;
...
void distribute_thread(LAYER &layer, LAYER &next_layer) {
  ...
  for (int i = 0; i < thread_num; i++)
    if (i < thread_num - 1)
      layer.mthreads[i] =
          new thread(parallel_comp, ref(layer), i, ref(next_layer));
    else
      layer.mthreads[i] =
          new thread(parallel_tail, ref(layer), i, ref(next_layer));
